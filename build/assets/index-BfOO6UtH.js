import {
  g as Ce,
  i as ke,
  a as Ge,
  b as X,
  c as ze,
  G as Ke,
  o as $,
  s as ee,
  d as J,
  e as Ye,
  f as Xe,
  h as _,
  q as je,
  w as he,
  T as ye,
  j as Te,
  k as Ze,
  u as _e,
  l as Ne,
  m as Qe,
  n as et,
  p as Se,
  E as tt,
  r as st,
} from './firebase-wawmpWM2.js';
import './vite-plugin-node-polyfills-CYF0W5wK.js';
import { r as n, j as e, d as it, b as nt } from './vendor-BEztx3UH.js';
import { B as ot } from './react-router-dom-Djf-viZn.js';
import { P as c } from './prop-types-BiVUseAU.js';
import { D as at } from './react-datepicker-C0NVZxc2.js';
import { v as rt } from './uuid-BKT09osN.js';
import { u as A, a as ct, b as P } from './react-router-BqYFP2zy.js';
import './tslib-BGVaTf34.js';
import './idb-BXWtuYvb.js';
import './scheduler-DYLXRpC5.js';
import './@remix-run-BIrGyY9Z.js';
import './clsx-B-dksMZM.js';
import './date-fns-DXCimDFB.js';
import './react-onclickoutside-D9wCJI1E.js';
import './@floating-ui-C_tFKrs3.js';
(function () {
  const i = document.createElement('link').relList;
  if (i && i.supports && i.supports('modulepreload')) return;
  for (const o of document.querySelectorAll('link[rel="modulepreload"]')) a(o);
  new MutationObserver((o) => {
    for (const r of o)
      if (r.type === 'childList')
        for (const l of r.addedNodes)
          l.tagName === 'LINK' && l.rel === 'modulepreload' && a(l);
  }).observe(document, { childList: !0, subtree: !0 });
  function s(o) {
    const r = {};
    return (
      o.integrity && (r.integrity = o.integrity),
      o.referrerPolicy && (r.referrerPolicy = o.referrerPolicy),
      o.crossOrigin === 'use-credentials'
        ? (r.credentials = 'include')
        : o.crossOrigin === 'anonymous'
          ? (r.credentials = 'omit')
          : (r.credentials = 'same-origin'),
      r
    );
  }
  function a(o) {
    if (o.ep) return;
    o.ep = !0;
    const r = s(o);
    fetch(o.href, r);
  }
})();
const De = {
    apiKey: 'AIzaSyAYV9vGuuJW0As2r6kZnDmEXdRWPBWWR4c',
    authDomain: 'reminder5-27ef0.firebaseapp.com',
    projectId: 'reminder5-27ef0',
    storageBucket: 'reminder5-27ef0.firebasestorage.app',
    messagingSenderId: void 0,
    appId: '1:230818023391:web:1813670f0600a50839266a',
    measurementId: 'G-41WKE3E4T3',
    databaseURL: 'https://reminder3-65e84-default-rtdb.firebaseio.com',
  },
  we = Ce().length ? Ge() : ke(De),
  O = X(we),
  E = ze(we);
new Ke();
function lt() {
  const t = JSON.parse(localStorage.getItem('tasks')) || [];
  t.forEach((i) => {
    const s = new Date(i.notificationTime).getTime(),
      a = new Date().getTime(),
      o = i.isNotified;
    if (s <= a && !o) {
      dt(i);
      const r = t.filter((l) => l.id !== i.id);
      localStorage.setItem('tasks', JSON.stringify(r));
    }
  });
}
const dt = (t) => {
  Notification.permission === 'granted'
    ? new Notification('Reminder', {
        body: `Task: ${t.content}`,
        icon: '/favicon.png',
        tag: 'unique-notification-id',
      })
    : Notification.permission !== 'denied' &&
      Notification.requestPermission().then((i) => {
        i === 'granted' &&
          new Notification('Reminder', {
            body: `Task: ${t.content}`,
            tag: 'unique-notification-id',
          });
      });
};
setInterval(lt, 6e4);
const Ee = n.createContext(),
  Ae = n.createContext(),
  ut = [
    {
      title: 'Make a restaurant reservation',
      description: 'user tasks',
      type: 'string',
      id: 1,
      content: 'Make a restaurant reservation',
      editing: !1,
      completed: !1,
      reserve: !1,
      editingLock: !1,
      editingColor: !1,
      editingDateTime: !1,
      notification: !1,
      shouldHandleNotifications: !1,
    },
    {
      title: 'send a letter',
      description: 'user tasks',
      type: 'string',
      id: 2,
      content: 'send a letter',
      editing: !1,
      completed: !1,
      reserve: !1,
      editingLock: !1,
      editingColor: !1,
      editingDateTime: !1,
      notification: !1,
      shouldHandleNotifications: !1,
    },
    {
      title: 'buy flowers',
      description: 'user tasks',
      type: 'string',
      id: 3,
      content: 'buy flowers',
      editing: !1,
      completed: !1,
      reserve: !1,
      editingLock: !1,
      editingColor: !1,
      editingDateTime: !1,
      notification: !1,
      shouldHandleNotifications: !1,
    },
  ],
  mt = (t, i) => {
    switch (i.type) {
      case 'todo/add':
        return [...t, i.todo];
      case 'todo/delete':
        return t.filter((s) => s.id !== i.todo.id);
      case 'todo/update':
        return t.map((s) =>
          s.id === i.todo.id ? { ...s, ...i.todo } : { ...s }
        );
      case 'todo/complete':
        return t.map((s) =>
          s.id === i.todo.id ? { ...s, ...i.todo } : { ...s }
        );
      case 'todo/reset':
        return [];
      case 'todo/reserve':
        return t.map((s) =>
          s.id === i.todo.id
            ? { ...s, editingLock: !0 }
            : { ...s, editingLock: !1 }
        );
      case 'todo/reserveColor':
        return t.map((s) =>
          s.editingDateTime
            ? { ...s, editingColor: !0 }
            : { ...s, editingColor: !1 }
        );
      case 'todo/editingDateTime':
        return t.map((s) =>
          s.editingLock
            ? { ...s, editingDateTime: !0 }
            : { ...s, editingDateTime: !1 }
        );
      case 'FETCH_TODOS':
        return i.payload;
      case 'complete2':
        return t.map((s) =>
          s.id === i.todo.id ? { ...s, completed: !0 } : { ...s }
        );
      case 'todo/notification':
        return t.map((s) =>
          s.id === i.todo.id
            ? { ...s, notification: !0, shouldHandleNotifications: !0 }
            : { ...s }
        );
      default:
        return t;
    }
  },
  Ie = ({ children: t }) => {
    const [i, s] = n.useReducer(mt, { todoList: [] }),
      [a, o] = n.useState([]),
      [r, l] = n.useState(!1),
      [g, f] = n.useState(!1),
      [T, x] = n.useState(!1),
      [m, p] = n.useState(!1),
      [S, D] = n.useState(!1),
      [v, R] = n.useState(!1),
      [q, C] = n.useState(!1),
      [y, N] = n.useState(!1),
      [h, d] = n.useState(!1),
      [j, u] = n.useState(''),
      [I, Z] = n.useState(''),
      [z, M] = n.useState(!1),
      [F, se] = n.useState(!1),
      [B, ie] = n.useState(!1),
      [ne, oe] = n.useState(!1),
      [K, ae] = n.useState(),
      [w, re] = n.useState(),
      [ce, le] = n.useState(),
      [de, ue] = n.useState(),
      [me, U] = n.useState(),
      [H, pe] = n.useState(),
      [ge, b] = n.useState(),
      [k, W] = n.useState(),
      [G, V] = n.useState(),
      [fe, He] = n.useState(!1),
      [We, Ve] = n.useState(),
      [$e, Je] = n.useState(!1);
    return e.jsx(Ee.Provider, {
      value: {
        todos: i,
        isDateChecked: T,
        setIsDateChecked: x,
        isTimeChecked: m,
        setIsTimeChecked: p,
        isContainerTimeCheck: v,
        setContainerTimeCheck: R,
        isContainerDateCheck: S,
        setContainerDateCheck: D,
        modalOpen: r,
        setModalOpen: l,
        displayTimePicker: y,
        setDisplayTimePicker: N,
        displayDatePicker: h,
        setDisplayDatePicker: d,
        isDateSet: g,
        setIsDateSet: f,
        isTimeSet: q,
        setIsTimeSet: C,
        enteredTodo: j,
        setEnteredTodo: u,
        fireTodo: I,
        setFireTodo: Z,
        todosData: a,
        setTodosData: o,
        todoList: ut,
        selectedDate: z,
        setSelectedDate: M,
        selectedTime: F,
        setSelectedTime: se,
        AddTodosExecuted: ne,
        setAddTodosExecuted: oe,
        completedDateTimeSetting: B,
        setCompletedDateTimeSetting: ie,
        notificationDocId: K,
        setNotificationDocId: ae,
        isSubmitting: w,
        setIsSubmitting: re,
        isDocRef: de,
        setIsDocRef: ue,
        reserveModeTodo: me,
        setReserveModeTodo: U,
        reserveModeId: H,
        setReserveModeId: pe,
        todoId: ge,
        setTodoId: b,
        todoContent: k,
        setTodoContent: W,
        Todo: G,
        setTodo: V,
        shouldHandleNotifications: fe,
        setShouldHandleNotifications: He,
        isSubmitting2: ce,
        setIsSubmitting2: le,
        agree: We,
        setAgree: Ve,
        isSet: $e,
        setIsSet: Je,
      },
      children: e.jsx(Ae.Provider, { value: s, children: t }),
    });
  };
Ie.propTypes = { children: c.node.isRequired };
const L = () => n.useContext(Ee),
  te = () => n.useContext(Ae),
  Pe = n.createContext(),
  Re = ({ children: t }) => {
    const { todos: i, todoList: s, AddTodosExecuted: a } = L(),
      [o, r] = n.useState(),
      [l, g] = n.useState(!0),
      [f, T] = n.useState([]),
      [x, m] = n.useState(!1),
      [p, S] = n.useState(),
      D = te(),
      v = O.currentUser,
      [R, q] = n.useState(),
      C = n.useRef(null);
    n.useEffect(() => {
      const h = $(O, (d) => {
        d ? q(d.uid) : console.log('No user is signed in');
      });
      return () => h();
    }, []);
    const y = n.useMemo(
        () => ({
          toFirestore: (h) => {
            const d = Object.values(h),
              j = {};
            return (
              d.forEach((u, I) => {
                j[I.toString()] = {
                  title: u.title,
                  description: u.description,
                  type: u.type,
                  id: u.id,
                  content: u.content,
                  editing: u.editing,
                  completed: u.completed,
                  reserve: u.reserve,
                  editingLock: u.editingLock,
                  editingColor: u.editingColor,
                  editingDateTime: u.editingDateTime,
                  notification: u.notification,
                };
              }),
              j
            );
          },
        }),
        []
      ),
      N = n.useMemo(
        () => ({
          fromFirestore: (h) =>
            Object.values(h).map((u) => ({
              title: u.title,
              description: u.description,
              type: u.type,
              id: u.id,
              content: u.content,
              editing: u.editing,
              completed: u.completed,
              reserve: u.reserve,
              editingLock: u.editingLock,
              editingColor: u.editingColor,
              editingDateTime: u.editingDateTime,
              notification: u.notification,
            })),
        }),
        []
      );
    return (
      n.useEffect(() => {
        const h = async () => {
            if (!R) {
              console.log('');
              return;
            }
            try {
              const j = y.toFirestore(i),
                u = { todoId: v.uid, todos: j };
              await ee(J(E, 'todoList3', v.uid), u);
            } catch (j) {
              console.error('Error adding todoList to Firestore:', j);
            } finally {
              g(!1);
            }
          },
          d = $(O, (j) => {
            j
              ? h(i, j.uid)
              : console.log('User signed out or not yet logged in');
          });
        return () => d();
      }, [D, i, E, y]),
      n.useEffect(() => {
        const h = async (j) => {
            try {
              const u = J(E, 'todoList3', j),
                I = await Ye(u);
              if (I.exists()) {
                const z = I.data().todos || [],
                  M = N.fromFirestore(z);
                r(!0);
                const F = Array.isArray(M) ? M : [M];
                F !== null &&
                  F.length > 0 &&
                  (T(F),
                  r(!0),
                  (C.current = F),
                  D({ type: 'FETCH_TODOS', payload: F || [] }));
              }
            } catch (u) {
              console.error('Error fetching todoList to Firestore:', u);
            } finally {
              g(!1);
            }
          },
          d = $(O, (j) => {
            x ? console.log('User signed out') : h(j.uid);
          });
        return () => d();
      }, [N, D]),
      n.useEffect(() => {
        a &&
          (async () => {
            try {
              const d = y.toFirestore(i),
                j = { todoId: v.uid, todos: d };
              await ee(J(E, 'todoList3', v.uid), j);
            } catch (d) {
              console.error('Error adding todoList to Firestore:', d);
            } finally {
              g(!1);
            }
          })();
      }, [a, D, i, N, y]),
      e.jsx(Pe.Provider, {
        value: {
          data: o,
          setData: r,
          loading: l,
          setLoading: g,
          fetchedData: f,
          setFetchedData: T,
          todoList: s,
          user: v,
          uid: R,
          firestore: E,
          convertdedNotificationData: p,
          setComvertedNotificationData: S,
          setTodosChanged: m,
        },
        children: t,
      })
    );
  };
Re.propTypes = { children: c.node.isRequired };
const pt = () => n.useContext(Pe),
  Le = ({
    handleTimeCheckboxChange: t,
    shouldHandleNotifications: i,
    timeCheck: s,
  }) => {
    Le.propTypes = {
      handleTimeCheckboxChange: c.func.isRequired,
      shouldHandleNotifications: c.bool.isRequired,
      timeCheck: c.bool.isRequired,
    };
    const { isTimeChecked: a, setIsTimeChecked: o } = L();
    return (
      (t = () => {
        o(!a);
      }),
      e.jsx(e.Fragment, {
        children: e.jsxs('div', {
          className: 'inner-container',
          children: [
            e.jsx('h4', {
              className: 'time',
              style: {
                color: i && s ? 'rgb(8, 232, 158)' : 'rgb(48, 48, 219)',
              },
              children: 'TIME',
            }),
            e.jsxs('label', {
              className: 'switch2',
              children: [
                e.jsx('input', {
                  className: 'switch-time',
                  type: 'checkbox',
                  checked: a,
                  onChange: t,
                }),
                e.jsx('span', {
                  className: 'slider round',
                  style: { color: i ? 'rgb(8, 232, 158)' : 'rgb(48, 48, 219)' },
                }),
              ],
            }),
          ],
        }),
      })
    );
  },
  Y = ({
    handleTimeChange: t,
    inputTime: i,
    shouldHandleNotifications: s,
    timeCheck: a,
  }) => {
    Y.propTypes = {
      handleTimeChange: c.func.isRequired,
      inputTime: c.string.isRequired,
      shouldHandleNotifications: c.bool,
      timeCheck: c.bool,
    };
    const { setDisplayTimePicker: o, setDisplayDatePicker: r } = L();
    return (
      n.useEffect(() => {
        o(!0), r(!1);
      }, [o, r]),
      e.jsxs('div', {
        className: 'time-picker-container',
        style: {
          border: s && a ? '1px solid rgb(8, 232, 158)' : '1px solid #ccc',
          padding: '8px',
          borderRadius: '4px',
        },
        children: [
          e.jsx('h2', {
            style: { color: s && a ? 'rgb(8, 232, 158)' : 'rgb(48, 48, 219)' },
            children: 'Time Picker',
          }),
          e.jsx('input', {
            className: 'MyTimePicker',
            onChange: t,
            selected: i,
            label: 'MyTimePicker',
            type: 'time',
            value: i,
            style: {
              width: '160px',
              padding: '6px',
              borderRadius: '4px',
              border: '1px solid #ccc',
              fontSize: '16px',
            },
          }),
          e.jsxs('p', {
            style: { color: s && a ? 'rgb(8, 232, 158)' : ' rgb(48, 48, 219)' },
            children: ['Selected time: ', i],
          }),
        ],
      })
    );
  },
  Q = ({
    isDate: t,
    handleDateChange: i,
    shouldHandleNotifications: s,
    timeCheck: a,
  }) => {
    Q.propTypes = {
      isDate: c.instanceOf(Date).isRequired,
      handleDateChange: c.func.isRequired,
      shouldHandleNotifications: c.bool,
      timeCheck: c.bool,
    };
    const {
      selectedDate: o,
      setDisplayDatePicker: r,
      setDisplayTimePicker: l,
    } = L();
    return (
      n.useEffect(() => {
        r(!0), l(!1);
      }, [l, r]),
      e.jsxs('div', {
        className: 'date-picker-container',
        style: {
          border: s && a ? '1px solid rgb(8, 232, 158)' : '1px solid #ccc',
          padding: '8px',
          borderRadius: '4px',
        },
        children: [
          e.jsx('h2', {
            style: { color: s && a ? 'rgb(8, 232, 158)' : 'rgb(48, 48, 219)' },
            children: 'Date Picker',
          }),
          e.jsx(at, {
            className: 'DateTimePicker custom',
            label: 'DateTimePicker',
            inputVariant: 'outlined',
            onChange: i,
            selected: t,
            showTodayButton: !0,
            ampm: !1,
            autoOk: !0,
          }),
          o &&
            t &&
            e.jsxs('p', {
              style: {
                color: s && a ? 'rgb(8, 232, 158)' : ' rgb(48, 48, 219)',
              },
              children: ['Selected Date: ', t.toLocaleDateString()],
            }),
        ],
      })
    );
  },
  qe = ({
    handleDateCheckboxChange: t,
    shouldHandleNotifications: i,
    timeCheck: s,
  }) => {
    qe.propTypes = {
      handleDateCheckboxChange: c.func.isRequired,
      shouldHandleNotifications: c.bool.isRequired,
      timeCheck: c.bool.isRequired,
    };
    const { isDateChecked: a, setIsDateChecked: o } = L();
    return (
      (t = () => {
        o(!a);
      }),
      e.jsx(e.Fragment, {
        children: e.jsxs('div', {
          className: 'inner-container',
          children: [
            e.jsx('h4', {
              style: {
                color: i && s ? 'rgb(8, 232, 158)' : 'rgb(48, 48, 219)',
              },
              children: 'DATE',
            }),
            e.jsxs('label', {
              className: 'switch',
              children: [
                e.jsx('input', {
                  className: 'switch-date',
                  type: 'checkbox',
                  checked: a,
                  onChange: t,
                }),
                e.jsx('span', { className: 'slider round' }),
              ],
            }),
          ],
        }),
      })
    );
  };
Ce().length || ke(De);
const be = X(),
  xe = ({
    shouldHandleNotifications: t = !1,
    completedDateTimeSetting: i = !1,
    todo: s = {},
  }) => {
    (xe.propTypes = { todo: c.object }),
      (xe.propTypes = {
        todo: c.shape({
          id: c.number,
          content: c.string,
          editing: c.bool,
          editingColor: c.bool,
          completed: c.bool,
          editingDateTime: c.bool,
          editingLock: c.bool,
        }).isRequired,
        children: c.node,
        shouldHandleNotifications: c.bool,
        completedDateTimeSetting: c.bool,
      }),
      localStorage.clear();
    const [a, o] = n.useState();
    $(be, (l) => {
      l && l.getIdToken(!0);
    }),
      n.useEffect(() => {
        const l = $(be, (g) => {
          g && o(g.uid);
        });
        return () => l();
      }, []);
    const r = async (l) => {
      await _e(J(E, 'notifications', l), { isNotified: !0 });
    };
    return (
      n.useEffect(() => {
        if (i && t && s) {
          const l = async () => {
              const f = _(E, 'notifications'),
                T = je(
                  f,
                  he('notificationTime', '>=', ye.now()),
                  he('isNotified', '==', !1)
                ),
                m = (await Te(T)).docs.map((p) => ({
                  ...p.data(),
                  id: p.id,
                  notificationTime: p.data().notificationTime.toDate(),
                  isNotified: p.data().isNotified,
                }));
              for (const p of m)
                p.isNotified ||
                  (await r(p.id),
                  localStorage.setItem('tasks', JSON.stringify(m)));
            },
            g = Xe(_(E, 'notifications'), (f) => {
              const x = f.docs
                .map((m) => ({
                  ...m.data(),
                  id: m.id,
                  notificationTime: m.data().notificationTime.toDate(),
                  isNotified: m.data().isNotified,
                }))
                .filter((m) => !m.isNotified);
              localStorage.setItem('tasks', JSON.stringify(x));
            });
          return l(), () => g();
        }
      }, [i, t, a, s]),
      n.useEffect(() => {
        (async () => {
          const g = _(E, 'notifications'),
            f = je(g, he('notificationTime', '<', ye.now())),
            T = await Te(f);
          for (const x of T.docs) {
            const m = x.id,
              S = x.data().notificationTime.toDate(),
              D = new Date().getTime();
            new Date(S).getTime() + 24 * 60 * 60 * 1e3 <= D &&
              (await Ze(J(E, 'notifications', m)));
          }
        })();
      }, []),
      null
    );
  },
  Fe = ({ todo: t }) => {
    Fe.propTypes = {
      todo: c.shape({
        id: c.number,
        content: c.string,
        editing: c.bool,
        editingColor: c.bool,
        completed: c.bool,
        editingDateTime: c.bool,
        editingLock: c.bool,
      }).isRequired,
      children: c.node,
    };
    const i = te(),
      [s, a] = n.useState(new Date()),
      [o, r] = n.useState(new Date()),
      [l, g] = n.useState(''),
      [f, T] = n.useState(s),
      [x, m] = n.useState(o),
      [p, S] = n.useState(!1),
      [D, v] = n.useState(!0),
      [R, q] = n.useState(),
      {
        isDateChecked: C,
        isTimeChecked: y,
        setIsDateChecked: N,
        setIsTimeChecked: h,
        isContainerDateCheck: d,
        setContainerDateCheck: j,
        isContainerTimeCheck: u,
        setContainerTimeCheck: I,
        modalOpen: Z,
        setIsTimeSet: z,
        setIsDateSet: M,
        setSelectedDate: F,
        setSelectedTime: se,
        completedDateTimeSetting: B,
        setCompletedDateTimeSetting: ie,
        setNotificationDocId: ne,
        isSubmitting: oe,
        setIsSubmitting: K,
        setIsDocRef: ae,
        shouldHandleNotifications: w,
        setShouldHandleNotifications: re,
      } = L();
    n.useEffect(() => {
      const b = $(O, (k) => {
        k && q(k.uid);
      });
      return () => b();
    }, []),
      n.useEffect(() => {
        'Notification' in window || v(!1);
      }, []);
    const ce = () => {
        if (!D)
          return e.jsx('div', {
            className: 'warning-message',
            children: e.jsx('p', {
              className: 'warning',
              children: 'This browser does not supported notifications',
            }),
          });
      },
      le = (b) => {
        N(!b);
      },
      de = (b) => {
        h(!b);
      },
      ue = (b) => {
        j(!0), I(!1), F(!0);
      },
      me = (b) => {
        I(!0), j(!1), se(!0);
      },
      U = (b) => {
        const k = new Date(b.target.value);
        a(k), M(!0);
      },
      H = (b) => {
        const { value: k } = b.target,
          [W, G] = k.split(':'),
          V = new Date();
        V.setHours(parseInt(W), parseInt(G), 0, 0), r(V), g(k), z(!0), S(!0);
      },
      pe = async (b) => {
        if (!oe) {
          if ((K(!0), s && o && p && b.content && b.id)) {
            const k = new Date(s);
            k.setHours(o.getHours(), o.getMinutes(), 0, 0);
            try {
              const W = {
                  title: 'Reminder',
                  description:
                    'Time is approaching, receive to push notification..',
                  type: 'string',
                  notificationTime: k,
                  todoId: R,
                  content: b.content,
                  id: b.id,
                  isNotified: !1,
                },
                G = rt(),
                V = J(_(E, 'notifications'), G);
              await ee(V, W);
              const fe = { ...b, notification: !0 };
              i({ type: 'todo/notification', todo: fe }),
                ne(G),
                ae(V),
                ie(!0),
                re(!0);
            } catch (W) {
              console.error(
                'Error writing notification data to Firestore: ',
                W
              );
            } finally {
              K(!1);
            }
          } else K(!1);
          try {
            const k = await Notification.requestPermission();
            k === 'granted'
              ? console.log('')
              : console.warn(
                  k === 'denied'
                    ? 'Notification permission denied'
                    : 'Notification permission dismissed'
                );
          } catch (k) {
            console.error('Failed to request permission:', k);
          }
        }
      };
    n.useEffect(() => {
      const b = document.querySelector('.modal');
      (b.style.alignItems = 'center'),
        (s !== f || o !== x) && (s !== f || o !== x) && (T(s), m(o));
    }, [s, o]);
    const ge = () => {
      if (w)
        return e.jsx('div', {
          className: 'successful',
          children: e.jsx('h3', {
            className: 'set-message',
            children: 'Completed Setting',
          }),
        });
    };
    return Z
      ? e.jsxs(
          'div',
          {
            className: 'modal',
            style: {
              backgroundColor:
                w && B && p ? 'transparent' : 'rgba(40, 147, 247, 0.772)',
              border:
                w && B && p
                  ? '1px solid rgb(8, 232, 158)'
                  : '1px solid rgb(48, 48, 219)',
            },
            children: [
              ce(),
              e.jsx('div', {
                className: 'switch-container',
                onClick: () => ue(),
                children: e.jsx(qe, {
                  handleDateCheckboxChange: le,
                  isChecked: C,
                  shouldHandleNotifications: w,
                  inputTime: l,
                  timeCheck: p,
                }),
              }),
              e.jsx('div', {
                className: 'switch-container2',
                onClick: () => me(),
                children: e.jsx(Le, {
                  handleTimeCheckboxChange: de,
                  isChecked: y,
                  shouldHandleNotifications: w,
                  inputTime: l,
                  timeCheck: p,
                }),
              }),
              C && y && u
                ? e.jsx(Y, {
                    className: 'MyTimePicker',
                    onChange: H,
                    selected: o,
                    label: 'MyTimePicker',
                    inputVariant: 'outlined',
                    showTodayButton: !0,
                    ampm: !1,
                    autoOk: !0,
                    isTime: o,
                    inputTime: l,
                    setInputTime: g,
                    handleTimeChange: H,
                    shouldHandleNotifications: w,
                    timeCheck: p,
                  })
                : C && y && d
                  ? e.jsx(Q, {
                      onChange: U,
                      selected: s,
                      isDate: s,
                      inputVariant: 'outlined',
                      handleDateChange: U,
                      shouldHandleNotifications: w,
                      inputTime: l,
                      timeCheck: p,
                    })
                  : C && y
                    ? e.jsx(Y, {
                        isTime: o,
                        handleTimeChange: H,
                        inputTime: l,
                        setInputTime: g,
                        inputVariant: 'outlined',
                        showTodayButton: !0,
                        ampm: !1,
                        autoOk: !0,
                      })
                    : C
                      ? e.jsx(Q, {
                          onChange: U,
                          selected: s,
                          isDate: s,
                          handleDateChange: U,
                        })
                      : y
                        ? e.jsx(Y, {
                            isTime: o,
                            handleTimeChange: H,
                            inputTime: l,
                            inputVariant: 'outlined',
                            showTodayButton: !0,
                            ampm: !1,
                            autoOk: !0,
                            setInputTime: g,
                          })
                        : d
                          ? e.jsx(Q, {
                              onChange: U,
                              selected: s,
                              isDate: s,
                              inputVariant: 'outlined',
                              handleDateChange: U,
                            })
                          : u
                            ? e.jsx(Y, {
                                isTime: o,
                                handleTimeChange: H,
                                inputTime: l,
                                inputVariant: 'outlined',
                                showTodayButton: !0,
                                ampm: !1,
                                autoOk: !0,
                                setInputTime: g,
                              })
                            : e.jsx(e.Fragment, {
                                children: e.jsxs('div', {
                                  className: 'message-container',
                                  style: {
                                    backgroundColor:
                                      !d && !u && w && B && p
                                        ? 'rgb(8, 232, 158)'
                                        : 'rgb(48, 48, 219)',
                                  },
                                  children: [
                                    e.jsx('h2', {
                                      className: 'message',
                                      children: 'Please Select',
                                    }),
                                    e.jsx('h2', {
                                      className: 'message',
                                      children: ' Setting',
                                    }),
                                    e.jsx('h2', {
                                      className: 'message',
                                      children: ' Date & Time',
                                    }),
                                  ],
                                }),
                              }),
              e.jsxs('div', {
                className: 'btn-container',
                children: [
                  e.jsx('button', {
                    className: 'set-btn',
                    onClick: () => pe(t),
                    style: {
                      color:
                        w && p
                          ? 'rgb(8, 232, 158)'
                          : 'rgb(40, 147, 247, 0.772)',
                    },
                    children: w ? 'DONE' : 'SET',
                  }),
                  B &&
                    w &&
                    e.jsx(xe, {
                      shouldHandleNotifications: w,
                      completedDateTimeSetting: B,
                      todo: t,
                    }),
                ],
              }),
              ge(),
            ],
          },
          t.id
        )
      : null;
  },
  Oe = ({ todo: t }) => {
    Oe.propTypes = {
      todo: c.shape({
        id: c.number.isRequired,
        content: c.string.isRequired,
        editing: c.bool.isRequired,
        editingColor: c.bool,
        completed: c.bool,
        editingDateTime: c.bool,
        editingLock: c.bool,
      }).isRequired,
      children: c.node,
    };
    const {
        setModalOpen: i,
        setReserveModeTodo: s,
        setReserveModeId: a,
        setIsDateChecked: o,
        setIsTimeChecked: r,
        setContainerDateCheck: l,
        setContainerTimeCheck: g,
        modalOpen: f,
        Todo: T,
        setTodo: x,
        setShouldHandleNotifications: m,
        isSet: p,
      } = L(),
      [S, D] = n.useState(t.content),
      v = te(),
      R = (d) => {
        D(d.target.value);
      },
      q = () => {
        const d = { ...t, editing: !t.editing };
        v({ type: 'todo/update', todo: d });
      },
      C = (d) => {
        d.preventDefault();
        const j = { ...t, editing: !t.editing, content: S };
        v({ type: 'todo/update', todo: j });
      },
      y = (d) => {
        v({ type: 'todo/delete', todo: d });
      },
      N = (d) => {
        const j = { ...d, completed: !0 };
        v({ type: 'complete2', todo: j });
      },
      h = (d) => {
        const j = !d.editingColor,
          u = {
            ...d,
            editingDateTime: !0,
            editingColor: j,
            editingLock: !0,
            id: d.id,
            content: d.content,
          };
        v({ type: 'todo/reserve', todo: u }),
          v({ type: 'todo/reserveColor', todo: u }),
          v({ type: 'todo/editingDateTime', todo: u }),
          i((I) => !I),
          s(d),
          a(d.id),
          x(d),
          !f && o(!1),
          !f && r(!1),
          !f && l(!1),
          !f && g(!1),
          !f && m(!1);
      };
    return e.jsxs(
      'div',
      {
        className: 'modalParent',
        children: [
          e.jsx('span', {
            className: 'circleI',
            onClick: () => h(t),
            style: {
              color:
                p && t.editingColor && t.editingDateTime && t.editingLock
                  ? 'yellow'
                  : 'grey',
            },
          }),
          e.jsx('button', {
            className: 'compBtn',
            onClick: () => N(t),
            onDoubleClick: () => y(t),
            style: { color: t.completed ? 'rgb(8, 232, 158)' : 'none' },
            children: t.completed ? 'Completed' : 'Complete',
          }),
          e.jsx('form', {
            onSubmit: C,
            style: { display: 'inline' },
            children: t.editing
              ? e.jsx('input', { type: 'text', value: S, onChange: R })
              : e.jsx('span', {
                  className: 'content',
                  onDoubleClick: q,
                  style: {
                    textDecoration: t.completed ? 'line-through' : 'none',
                    color: t.completed ? 'rgb(8, 232, 158)' : 'none',
                  },
                  children: t.content,
                }),
          }),
          f && t.id == T.id
            ? e.jsx(
                'div',
                {
                  className: 'modal-container',
                  children: e.jsx(Fe, { todo: T }),
                },
                t.id
              )
            : null,
        ],
      },
      t.id
    );
  },
  gt = () => {
    const { todos: t } = L(),
      { data: i, loading: s } = pt();
    return e.jsx(e.Fragment, {
      children: s
        ? e.jsx('div', { children: 'Loading...' })
        : e.jsx('div', {
            children:
              Array.isArray(t) &&
              t.length > 0 &&
              i &&
              t &&
              t.map((a) => (a && a.id ? e.jsx(Oe, { todo: a }, a.id) : null)),
          }),
    });
  },
  ft = () => {
    const t = te(),
      { enteredTodo: i, setEnteredTodo: s, setAddTodosExecuted: a } = L(),
      o = () => {
        const l = {
          title: i,
          description: i,
          type: 'string',
          id: Math.floor(Math.random() * 1e5),
          content: i,
          editing: !1,
          completed: !1,
          reserve: !1,
          editingLock: !1,
          editingColor: !1,
          editingDateTime: !1,
          notification: !1,
        };
        t({ type: 'todo/add', todo: l, editing: !1 }), s(''), a(!0);
      },
      r = () => {
        t({ type: 'todo/reset', todo: [] });
      };
    return e.jsxs('div', {
      children: [
        e.jsx('input', {
          type: 'text',
          value: i,
          id: 'task',
          name: 'task',
          onChange: (l) => {
            s(l.target.value);
          },
        }),
        e.jsxs('div', {
          className: 'flex-box',
          children: [
            e.jsx('button', {
              className: 'add',
              onClick: () => o(),
              children: e.jsxs('div', {
                className: 'plus',
                children: [
                  e.jsx('span', { className: 'gif1' }),
                  e.jsx('span', { className: 'gif2' }),
                ],
              }),
            }),
            e.jsx('button', {
              className: 'reset2',
              onClick: () => r(),
              children: e.jsx('div', {
                className: 'reset',
                children: e.jsx('img', {
                  src: '/icon_007476_32.png',
                  alt: 'reset',
                }),
              }),
            }),
          ],
        }),
      ],
    });
  },
  ht = () => e.jsxs(e.Fragment, { children: [e.jsx(gt, {}), e.jsx(ft, {})] }),
  xt = () => {
    const { modalOpen: t, timeCheck: i } = L(),
      s = [1, 2, 3, 4, 5],
      a = () =>
        i && t
          ? 'rgb(8, 232, 158)'
          : t
            ? 'rgba(40, 147, 247, 0.772)'
            : 'rgb(8, 232, 158)';
    return e.jsxs(e.Fragment, {
      children: [
        e.jsx('div', { className: 'triangle' }),
        e.jsx('div', {
          className: 'decoration',
          children: s.map((o, r) =>
            e.jsx(
              'span',
              { className: 'slash', style: { backgroundColor: a() } },
              r
            )
          ),
        }),
        e.jsx('div', {
          className: 'container',
          children: e.jsxs('section', {
            children: [
              e.jsx('div', {
                className: 'big-title',
                children: e.jsx('h1', {
                  className: 'big-text',
                  style: {
                    color:
                      i && t
                        ? 'rgb(8, 232, 158)'
                        : t
                          ? 'rgba(40, 147, 247, 0.772)'
                          : 'rgb(8, 232, 158)',
                  },
                  children: 'REMINDER',
                }),
              }),
              e.jsx('div', {
                className: 'clone-big-title',
                children: e.jsx('h1', {
                  className: 'clone-text',
                  children: 'REMINDER',
                }),
              }),
              e.jsx('div', {
                className: 'discription',
                children: e.jsx('p', {
                  children:
                    "Manage your tasks and make your lifestyle meaningful with the Reminder App. Let's get started. You can input your requirements and add them by pressing the plus button. Once your tasks are successfully executed and completed, press the complete button. You can delete a task by double-clicking it. Once all tasks are completed, you can clear them using the refresh button. Be careful, as pressing the refresh button is irreversible. Have a great lifestyle!",
                }),
              }),
              e.jsxs('div', {
                className: 'out-line',
                children: [
                  e.jsx('h2', { className: 'title', children: 'Reminder' }),
                  e.jsx(ht, {}),
                ],
              }),
            ],
          }),
        }),
      ],
    });
  },
  jt = () => {
    const t = A(),
      i = () => {
        t('/UserAuth');
      };
    return e.jsxs(e.Fragment, {
      children: [
        e.jsx('span', {
          className: 'back',
          onClick: i,
          children: 'Back to Auth',
        }),
        e.jsx(Re, { children: e.jsx(xt, {}) }),
      ],
    });
  },
  Ue = () =>
    e.jsx(e.Fragment, { children: e.jsx(Ie, { children: e.jsx(jt, {}) }) }),
  yt = X();
function Me() {
  return new Promise((t) => {
    const i = $(yt, (s) => {
      t(!!s), i();
    });
  });
}
function Tt({ children: t }) {
  const i = A();
  return (
    n.useEffect(() => {
      Me().then((s) => {
        s || i('/UserAuth');
      });
    }, [i]),
    t
  );
}
const bt = X(),
  Be = () => {
    const t = A();
    let i = 5,
      s = [];
    for (let p = 0; p < i; p++) s.push(p);
    const [a, o] = n.useState(!1),
      r = () => '#fff',
      l = () => {
        t('/SignUp');
      },
      g = () => {
        t('/SignIn');
      },
      f = () => {
        Ne(bt)
          .then(() => {
            o(!0), localStorage.clear();
          })
          .catch((p) => {
            console.error('ログアウトエラー:', p);
          });
      },
      T = () => {
        if (a)
          return e.jsx('div', {
            className: 'sign-out2',
            children: e.jsx('h3', { children: 'Signed Out successfully' }),
          });
        setTimeout(() => o(!1), 5e3);
      },
      x = () => {
        t('/EasyLogin');
      },
      m = () => {
        t('/DeleteAccount');
      };
    return e.jsxs(e.Fragment, {
      children: [
        e.jsx('div', {
          className: 'decoration',
          children: s.map((p, S) =>
            e.jsx(
              'span',
              { className: 'slash', style: { backgroundColor: r() } },
              S
            )
          ),
        }),
        e.jsxs('div', {
          className: 'container',
          children: [
            e.jsx('img', { className: 'logo', src: '/logo_transparent.png' }),
            e.jsx('div', { className: 'decoration-container' }),
            e.jsx('div', {
              className: 'circle2',
              children: e.jsxs('div', {
                className: 'inner9-container',
                children: [
                  e.jsx('span', {
                    id: 'SU',
                    className: 'select-auth sign-up',
                    onClick: () => l(),
                    children: 'Sign Up',
                  }),
                  e.jsx('span', {
                    id: 'SI',
                    className: 'select-auth sign-in',
                    onClick: () => g(),
                    children: 'Sign In',
                  }),
                  e.jsx('span', {
                    id: 'SO',
                    className: 'select-auth sign-out',
                    onClick: () => f(),
                    children: 'Sign Out',
                  }),
                  e.jsx('span', {
                    className: 'select-auth easy-login',
                    onClick: () => x(),
                    children: 'お試しログイン',
                  }),
                  e.jsx('span', {
                    id: 'DA',
                    className: 'select-auth delete-account',
                    onClick: () => m(),
                    children: 'Delete Account',
                  }),
                ],
              }),
            }),
            T(),
          ],
        }),
      ],
    });
  };
function vt() {
  const t = A(),
    [i, s] = n.useState(!1);
  return (
    n.useEffect(() => {
      Me().then((a) => {
        a ? s(!0) : t('/UserAuth');
      });
    }, [t]),
    e.jsx(e.Fragment, {
      children: e.jsx('div', {
        children: e.jsx(Tt, { children: i ? e.jsx(Ue, {}) : e.jsx(Be, {}) }),
      }),
    })
  );
}
function Ct() {
  const t = A();
  n.useEffect(() => {
    (async () => {
      await Ne(O), setTimeout(() => t('/UserAuth'), 1e4);
    })(),
      localStorage.clear();
  }, [t]);
}
const kt = [
  {
    title: 'Make a restaurant reservation',
    description: 'user tasks',
    type: 'string',
    id: 1,
    content: 'Make a restaurant reservation',
    editing: !1,
    completed: !1,
    reserve: !1,
    editingLock: !1,
    editingColor: !1,
    editingDateTime: !1,
    notification: !1,
  },
  {
    title: 'send a letter',
    description: 'user tasks',
    type: 'string',
    id: 2,
    content: 'send a letter',
    editing: !1,
    completed: !1,
    reserve: !1,
    editingLock: !1,
    editingColor: !1,
    editingDateTime: !1,
    notification: !1,
  },
  {
    title: 'buy flowers',
    description: 'user tasks',
    type: 'string',
    id: 3,
    content: 'buy flowers',
    editing: !1,
    completed: !1,
    reserve: !1,
    editingLock: !1,
    editingColor: !1,
    editingDateTime: !1,
    notification: !1,
  },
];
function Nt() {
  const [t, i] = n.useState(!1),
    s = n.useMemo(
      () => ({
        toFirestore: (C) => {
          const y = Object.values(C),
            N = {};
          return (
            y.forEach((h, d) => {
              N[d.toString()] = {
                title: h.title,
                description: h.description,
                type: h.type,
                id: h.id,
                content: h.content,
                editing: h.editing,
                completed: h.completed,
                reserve: h.reserve,
                editingLock: h.editingLock,
                editingColor: h.editingColor,
                editingDateTime: h.editingDateTime,
                notification: h.notification,
              };
            }),
            N
          );
        },
      }),
      []
    ),
    [a, o] = n.useState({ name: '', email: '', password: '' }),
    { name: r, email: l, password: g } = a,
    f = A(),
    T = [1, 2, 3, 4, 5],
    x = () => 'rgba(40, 147, 247, 0.772)',
    m = (C) => {
      o({ ...a, [C.target.id]: C.target.value });
    },
    p = () => {
      i(!0);
    },
    S = () => {
      f('/Terms');
    },
    D = () => {
      f('/Terms2');
    },
    v = () => {
      f('/PrivacyPolicy');
    },
    R = () => {
      f('/UserAuth');
    },
    q = async (C) => {
      C.preventDefault();
      try {
        if (g.length < 6)
          throw new Error('Password must be at least 6 characters long.');
        if (t) {
          const N = (await Qe(O, l, g)).user;
          await et(N, { displayName: r });
          const h = s.toFirestore(kt),
            d = { todoId: N.uid, agreement: t, todos: h };
          await ee(J(E, 'todoList3', N.uid), d), f('/Example');
        } else
          return e.jsx('div', {
            children: e.jsx('h2', {
              children: 'Agreement Required 利用規約に同意が必要です。',
            }),
          });
      } catch (y) {
        alert(y.message),
          y.code === 'auth/email-already-in-use'
            ? alert(
                'This email is already in use. Please use a different email.'
              )
            : (console.error('Error signing up:', y.message), alert(y.message));
      }
    };
  return e.jsxs(e.Fragment, {
    children: [
      e.jsx('div', { className: 'triangle' }),
      e.jsx('div', {
        className: 'decoration',
        children: T.map((C, y) =>
          e.jsx(
            'span',
            { className: 'slash', style: { backgroundColor: x() } },
            y
          )
        ),
      }),
      e.jsx('span', {
        className: 'back',
        onClick: R,
        children: 'Back To Auth',
      }),
      e.jsx('div', {
        className: 'auth-container',
        children: e.jsx('div', {
          className: 'outline-container',
          children: e.jsxs('form', {
            onSubmit: q,
            children: [
              e.jsx('input', {
                type: 'text',
                placeholder: 'Name',
                id: 'name',
                value: r,
                required: !0,
                onChange: m,
                className: 'form-input',
                autoComplete: 'name',
              }),
              e.jsx('input', {
                type: 'email',
                placeholder: 'Email',
                id: 'email',
                value: l,
                required: !0,
                onChange: m,
                className: 'form-input',
                autoComplete: 'email',
              }),
              e.jsx('input', {
                type: 'password',
                placeholder: 'Password',
                id: 'password',
                value: g,
                required: !0,
                onChange: m,
                className: 'form-input',
                autoComplete: 'current-password',
              }),
              e.jsxs('ul', {
                className: 'agreement-resource',
                children: [
                  e.jsx('li', {
                    className: 'terms',
                    onClick: () => S(),
                    children: 'Terms of Service Japanese',
                  }),
                  e.jsx('li', {
                    className: 'terms2',
                    onClick: () => D(),
                    children: 'Terms of Service English',
                  }),
                  e.jsx('li', {
                    className: 'policy',
                    onClick: () => v(),
                    children: 'PrivacyPolicy',
                  }),
                  e.jsx('p', {
                    className: 'request',
                    children: 'Request Agreement for Terms of Service',
                  }),
                ],
              }),
              e.jsxs('div', {
                className: 'inner-container2',
                children: [
                  e.jsx('input', {
                    type: 'checkbox',
                    className: 'agree-check',
                    onClick: () => p(),
                  }),
                  e.jsx('div', {
                    className: 'agree-container',
                    children: e.jsx('span', {
                      className: 'important',
                      style: {
                        color: t
                          ? 'rgb(8, 232, 158)'
                          : 'rgba(40, 147, 247, 0.772)',
                      },
                      children: t ? 'Agreed' : 'Agree',
                    }),
                  }),
                ],
              }),
              e.jsx('button', {
                type: 'submit',
                className: 'form-button',
                children: 'Submit',
              }),
            ],
          }),
        }),
      }),
    ],
  });
}
function St() {
  const [t, i] = n.useState({ email: '', password: '' }),
    { email: s, password: a } = t,
    o = A(),
    r = (x) => {
      i({ ...t, [x.target.id]: x.target.value });
    },
    l = async (x) => {
      x.preventDefault();
      try {
        (await Se(O, s, a)) && o('/Example');
      } catch (m) {
        console.log(m);
      }
    },
    g = () => 'rgba(40, 147, 247, 0.772)',
    f = [1, 2, 3, 4, 5],
    T = () => {
      o('/UserAuth');
    };
  return e.jsxs(e.Fragment, {
    children: [
      e.jsx('div', {
        className: 'decoration',
        children: f.map((x, m) =>
          e.jsx(
            'span',
            { className: 'slash2', style: { backgroundColor: g() } },
            m
          )
        ),
      }),
      e.jsx('span', {
        className: 'back',
        onClick: T,
        children: 'Back To Auth',
      }),
      e.jsx('div', {
        className: 'auth-container',
        children: e.jsx('div', {
          className: 'outline-container',
          children: e.jsxs('form', {
            onSubmit: l,
            children: [
              e.jsx('input', {
                type: 'email',
                placeholder: 'Email',
                id: 'email',
                value: s,
                required: !0,
                onChange: r,
                className: 'form-input',
                autoComplete: 'email',
              }),
              e.jsx('input', {
                type: 'password',
                placeholder: 'Password',
                id: 'password',
                value: a,
                required: !0,
                onChange: r,
                className: 'form-input',
                autoComplete: 'current-password',
              }),
              e.jsx('button', {
                type: 'submit',
                className: 'form-button',
                children: 'Submit',
              }),
            ],
          }),
        }),
      }),
    ],
  });
}
const ve = { email: 'Peace.875136D.time@gmail.com', password: '57tqw-7y6m' },
  Dt = () => {
    const t = A(),
      i = async () => {
        (await Se(O, ve.email, ve.password)) && t('/Example');
      };
    return (
      n.useEffect(() => {
        i();
      }, []),
      e.jsx('div', {
        className: 'login-container',
        children: e.jsx('div', {
          className: 'logging-in',
          children: 'Logging in...',
        }),
      })
    );
  },
  wt = () => {
    const [t, i] = n.useState(!1),
      [s, a] = n.useState(''),
      [o, r] = n.useState(''),
      g = X().currentUser,
      f = A(),
      T = async (m) => {
        if ((m.preventDefault(), g))
          try {
            const p = tt.credential(g.email, s);
            await st(g, p), await x();
          } catch (p) {
            r(`再認証中にエラーが発生しました: ${p.message}`);
          }
      },
      x = async () => {
        if (g)
          try {
            await g.delete(),
              r('ユーザーアカウントが削除されました'),
              setTimeout(() => f('/UserAuth'), 3e3);
          } catch (m) {
            m.code === 'auth/requires-recent-login'
              ? i(!0)
              : (r(`アカウント削除中にエラーが発生しました: ${m.message}`),
                setTimeout(() => f('/UserAuth', 1e4)));
          }
        else
          r('サインインしているユーザーがいません'),
            setTimeout(() => f('/UserAuth'), 1e4);
      };
    return (
      n.useEffect(() => {
        t || x();
      }, [t]),
      e.jsx('div', {
        children: t
          ? e.jsx('div', {
              className: 'auth-container',
              children: e.jsxs('div', {
                className: 'outline-container',
                children: [
                  e.jsx('h2', { children: '再認証' }),
                  e.jsxs('form', {
                    onSubmit: T,
                    children: [
                      e.jsx('div', {
                        children: e.jsxs('label', {
                          children: [
                            'Password:',
                            e.jsx('input', {
                              className: 'form-input',
                              type: 'password',
                              value: s,
                              onChange: (m) => a(m.target.value),
                              required: !0,
                            }),
                          ],
                        }),
                      }),
                      e.jsx('button', {
                        className: 'form-button',
                        type: 'submit',
                        children: 'Reauth',
                      }),
                    ],
                  }),
                  e.jsx('p', { children: o }),
                ],
              }),
            })
          : e.jsx('div', {
              className: 'auth-container',
              children: e.jsxs('div', {
                className: 'outline-container',
                children: [
                  e.jsx('h2', { children: 'アカウント削除中...' }),
                  e.jsx('p', { className: 'message', children: o }),
                ],
              }),
            }),
      })
    );
  },
  Et = () => {
    const t = A(),
      i = () => {
        t('/SignUp');
      };
    return e.jsxs('div', {
      className: 'container',
      children: [
        e.jsx('a', {
          href: 'https://www.privacypolicies.com/live/d955508c-375b-408d-8206-18852254e629',
          children: 'PrivacyPolicy',
        }),
        e.jsx('nav', { onClick: () => i(), children: 'Back to Sign Up' }),
      ],
    });
  },
  At = () => {
    const t = A(),
      i = () => {
        t('/SignUp');
      };
    return e.jsxs('div', {
      className: 'container',
      children: [
        e.jsx('embed', {
          src: '/VN3Licence-Ja.pdf',
          type: 'application/pdf',
          width: '100%',
          height: '1000px',
        }),
        e.jsx('nav', { onClick: () => i(), children: 'Back to Sign Up' }),
      ],
    });
  },
  It = () => {
    const t = A(),
      i = () => {
        t('/SignUp');
      };
    return e.jsxs('div', {
      className: 'container',
      children: [
        e.jsx('embed', {
          src: '/VN3Licence-En.pdf',
          type: 'application/pdf',
          width: '100%',
          height: '1000px',
        }),
        e.jsx('nav', { onClick: () => i(), children: 'Back to Sign Up' }),
      ],
    });
  };
function Pt() {
  return e.jsx('div', {
    className: 'App',
    children: e.jsx(ot, {
      future: { v7_startTransition: !0, v7_relativeSplatPath: !0 },
      children: e.jsxs(ct, {
        children: [
          e.jsx(P, { path: '/', element: e.jsx(vt, {}) }),
          e.jsx(P, { path: '/Example', element: e.jsx(Ue, {}) }),
          e.jsx(P, { path: '/UserAuth', element: e.jsx(Be, {}) }),
          e.jsx(P, { path: '/SignOut', element: e.jsx(Ct, {}) }),
          e.jsx(P, { path: '/SignUp', element: e.jsx(Nt, {}) }),
          e.jsx(P, { path: '/SignIn', element: e.jsx(St, {}) }),
          e.jsx(P, { path: '/EasyLogin', element: e.jsx(Dt, {}) }),
          e.jsx(P, { path: '/DeleteAccount', element: e.jsx(wt, {}) }),
          e.jsx(P, { path: '/PrivacyPolicy', element: e.jsx(Et, {}) }),
          e.jsx(P, { path: '/Terms', element: e.jsx(At, {}) }),
          e.jsx(P, { path: '/Terms2', element: e.jsx(It, {}) }),
        ],
      }),
    }),
  });
}
const Rt = it(document.getElementById('root'));
Rt.render(e.jsx(nt.StrictMode, { children: e.jsx(Pt, {}) }));
