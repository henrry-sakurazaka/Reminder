...
const Edit = ({ todo }) => {
  ...
  const {
    ...
    setIsDateChecked,
    setIsTimeChecked,
    setContainerDateCheck,
    setContainerTimeCheck,
    modalOpen,
    Todo,
    setTodo,
  ...
  return (
    <div key={todo.id} className="modalParent">
      <span
        className="circleI"
        onClick={() => toggleReseveMode(todo)}
        style={{
          color:
            isSet &&
            todo.editingColor &&
            todo.editingDateTime &&
            todo.editingLock
              ? 'yellow'
              : 'grey',
        }}
      ...
      <button
        className="compBtn"
        onClick={() => complete2(todo)}
        onDoubleClick={() => complete(todo)}
        style={{ color: todo.completed ? 'rgb(8, 232, 158)' : 'none' }}
      >
        ...
        {todo.editing ? (
          ...
          <span
            ...
            style={{
              ...
              color: todo.completed ? 'rgb(8, 232, 158)' : 'none',
            }}
          >
            {todo.content}
          </span>
